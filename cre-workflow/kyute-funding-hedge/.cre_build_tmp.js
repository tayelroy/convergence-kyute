...
var fetchHyperliquidFundingHistory = (requester, coin) => {
  ...
  const coinData = data.find((item) => item[0] === coin);
  if (!coinData)
    return 0;
  const venues = coinData[1];
  const hlPerpEntry = venues.find((v) => v[0] === "HlPerp");
  if (!hlPerpEntry)
    return 0;
  const fundingRate = Number(hlPerpEntry[1].fundingRate);
  return fundingRate * 24 * 365;
};
var fetchBorosImpliedApr = (requester, coin) => {
  return 0.15;
};
var predictFunding = (requester, hlApr, borosApr) => {
...
var onCronTrigger = (runtime2) => {
  ...
  const hlApr = http.sendRequest(runtime2, fetchHyperliquidFundingHistory, consensusIdenticalAggregation())(config.coin).result();
  const borosApr = http.sendRequest(runtime2, fetchBorosImpliedApr, consensusIdenticalAggregation())(config.coin).result();
  runtime2.log(`HL APR: ${(hlApr * 100).toFixed(2)}%`);
  runtime2.log(`Boros APR: ${(borosApr * 100).toFixed(2)}%`);
  const prediction = http.sendRequest(runtime2, predictFunding, consensusIdenticalAggregation())(hlApr, borosApr).result();
  runtime2.log(`Predicted APR: ${(prediction.apr * 100).toFixed(2)}%`);
  runtime2.log(`Confidence: ${prediction.confidence} bp`);
  const borosAprBp = Math.floor(borosApr * 1e4);
  const predictedAprBp = Math.floor(prediction.apr * 1e4);
  ...
  const proofHash = "0x" + Buffer.from("mock-zkp-hash-for-cre-simulation").toString("hex").padEnd(64, "0");
  ...
  const evmClient = new ClientCapability(4949039107694359620n);
  ...
  runtime2.log(`Hedge Decision Computed: ${shouldHedge}`);
